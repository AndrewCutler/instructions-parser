...
def main():
    ...
    parsed = []
    texts = ['Take 1 capsule a day by mouth as needed for pain.',
             '2 tablets orally q6h']
    for text in texts:
        ...
        for match_id, start, end in filtered_matches:
            ...
            if name == "DOSE_WITH_FORM":
                for token in matched_span:
                    if token.like_num:
                        dose_match = (token.idx, token.idx +
                                      len(token.text), 'DOSE')
                    if token._.is_dose_form:
                        dose_form_match = (
                            token.idx, token.idx + len(token.text), 'DOSE_FORM')
            if name == 'ROUTES':
                startIdx = matched_span[0].idx
                endIdx = startIdx + len(matched_span.text)
                route_match = (startIdx, endIdx, 'ROUTE')

        parsed.append(f'({text}, [{dose_match}, {dose_form_match}, {
            route_match}, {frequency_match}])')

    filepath = 'data-' + datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + '.txt'
